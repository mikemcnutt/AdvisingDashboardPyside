...
class StudentCard(QFrame):
    ...
    def _setup_ui(self):
        ...
        name_label.clicked.connect(lambda: self.openRequested.emit(self.student))
...
class AdvisingDashboard(QMainWindow):
    ...
    def _build_control_panel(self, parent_layout):
        ...
        self.email_body.setPlainText(saved_body or default_body)
    ...
    def _scan_folder(self):
        ...
        for jf in json_files:
            ...
            if not student_name:
                ...
                if student_name:
                    name_parts = student_name.split(None, 1)
            ...
            badges = []
            spring_done = summer_done = fall_done = False
            spring_partial = summer_partial = fall_partial = False
    ...
    def _populate_lists(self):
        ...
        for s in self.snapshots:
            ...
            if track_filter != "All Tracks" and track_filter_value(s) != track_filter:
                continue
    ...
    def _email_selected_needs(self, draft: bool):
        selected = [s for s in self.snapshots if str(s.file_path) in self.needs_checks and self.needs_checks[str(s.file_path)].isChecked()]
        ...
        if not draft:
            reply = QMessageBox.question(self, "Confirm", f"Send emails to {len(selected)} student(s)?", QMessageBox.Yes | QMessageBox.No)
